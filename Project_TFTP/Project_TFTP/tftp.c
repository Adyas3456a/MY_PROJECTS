...
int SendFile(int sockfd, struct sockaddr_in receiver_addr, tftp_packet *sender_pac, char *filename, char mode) 
{
    ...
    while (read(fd, &ch, 1)>0)
    {
        ...
        if (mode==OCTET_MODE)
        {
            ...
            if(ntohs(receiver_addr.sin_port)==PORT)    
...
int ReceiveFile(int sockfd, struct sockaddr_in sender_addr, tftp_packet *receiver_pac, int fd)
{
    ...
    while (1)
    {
        ...
        if (receiver_pac->opcode == DATA)
        {
            ...
            if(ntohs(sender_addr.sin_port)==PORT)
