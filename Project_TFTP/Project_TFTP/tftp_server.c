...
void client_handle(int sockfd, struct sockaddr_in client_addr, socklen_t client_len, tftp_packet *packet);
...
int main() 
{
    ...
    while (running) 
    {
        ...
        client_handle(sockfd, client_addr, client_len, &server_packet);
    }
    ...
    close(sockfd);
    pthread_cancel(exit_thread);  // Optional: force kill if not already returned
...
void client_handle(int sockfd, struct sockaddr_in client_addr, socklen_t client_len, tftp_packet *packet) 
{
    ...
    // and call SendFile or ReceiveFile accordingly
    if(packet->opcode==WRQ)
    {
        ...
        if(ReceiveFile(sockfd, client_addr, packet, fd)==FAILURE)
        {
    ...
    else if(packet->opcode==RRQ)
    {
        ...
        // printf("INFO: ACK send to client\n");
        sleep(1);
        if(SendFile(sockfd, client_addr, packet, filename, mode)==FAILURE)
        {
